...
export const getAll = async () => {
    ...
    const equiposConDatos = await Promise.all(equipos.map(async (equipo) => {
        if (equipo.firebase_uid) {
            try {
                const userRecord = await admin.auth().getUser(equipo.firebase_uid);
                return {
                    ...equipo,
                    nombre_creador: userRecord.displayName || userRecord.email?.split('@')[0] || 'Usuario',
                    email_creador: userRecord.email || ''
                };
            } catch (error) {
                console.error(`Error obteniendo datos de Firebase para UID ${equipo.firebase_uid}:`, error.message);
                return {
                    ...equipo,
...
async function eliminarImagenCloudinary(imageUrl) {
    try {
        ...
        // Ejemplo: https://res.cloudinary.com/tu-cloud/image/upload/v1234/equipos/equipo_123.jpg
        const matches = imageUrl.match(/\/v\d+\/(.+)\.\w+$/);
        if (matches && matches[1]) {
            const publicId = matches[1]; // equipos/equipo_123
            
            // Llamar al servicio de Cloudinary para eliminar
            const cloudinaryServiceUrl = process.env.CLOUDINARY_SERVICE_URL || 'http://localhost:3004';
            const encodedPublicId = publicId.replace(/\//g, '|'); // Reemplazar / por |
            
            await fetch(`${cloudinaryServiceUrl}/delete-imagen/${encodedPublicId}`, {
                method: 'DELETE'
            });
        ...
        // No lanzar error para que no bloquee la eliminación del equipo
    }
}
