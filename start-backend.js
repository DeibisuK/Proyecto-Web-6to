...
if (fs.existsSync(servicesRoot)) {
  ...
  services = fs
    .readdirSync(oscBackendRoot, { withFileTypes: true })
  ...
  return new Promise((resolve, reject) => {
    ...
    process.on("close", (code) => {
      if (code === 0) {
        ...
        console.error(
          `✗ "${command}" in ${serviceName} failed with code ${code}.`
        );
...
function checkEnvFiles() {
  ...
  for (const service of services) {
    const servicePath = fs.existsSync(servicesRoot)
      ? path.join(servicesRoot, service)
      : path.join(oscBackendRoot, service);
...
async function setupEnvFiles() {
  ...
  console.log(
    "💡 You will need to provide database credentials from DigitalOcean\n"
  );
...
function startAll() {
  ...
  services.forEach((service) => {
    ...
    try {
      ...
      console.error(
        `[${service}-ERROR] Failed to read package.json: ${e.message}`
      );
    }

    if (!commandParts) {
      if (fs.existsSync(path.join(servicePath, "src", "server.js")))
        commandParts = ["node", "src/server.js"];
      else if (fs.existsSync(path.join(servicePath, "server.js")))
        commandParts = ["node", "server.js"];
    }

    if (!commandParts) {
      console.log(
        `- Skipping ${service}: no start script or server file found.`
      );
      return;
    }
  ...
  console.log(
    "📍 Services started (ports depend on each service .env/config). Started services:"
  );
